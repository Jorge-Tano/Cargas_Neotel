  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
        ...
        {/* ── Columna izquierda: IDs de base de datos ── */}
              ...
              {ID_LABELS.map(({ key, caso, label }) => (
                <div key={key} className="flex items-center gap-4">
                  ...
                  <input
                    ...
                    className="w-28 rounded-xl border border-slate-200 px-3 py-2 text-sm font-mono text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-200 text-center"
                  />
                  <span className="text-xs text-slate-400 font-mono truncate">= {ids[key]}</span>
                </div>
              ))}
            </div>
        ...
        {/* ── Columna derecha: Rutas de salida ── */}
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
